...
describe('Testing EventBridge wrapper factory', function () {
    ...
    it("All properties get copied", async function () {
        ...
        const fac = new LambdaFactoryManager().eventBridgeWrapperFactory("ahandler");
        ...
        const { handler, configuration } = fac2.createHandler( class Ctrl {
            static async init() {
                return new Ctrl();
            }
            ahandler = ahandler
        ...
        await handler( Object.assign( testEventBridgeEvent, { "detail": { a:2 } }), LambdaContext, () => {})

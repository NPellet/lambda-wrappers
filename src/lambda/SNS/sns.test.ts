import { LambdaContext, testSNSEvent, testSNSRecord } from "../../test_utils/utils";
...
describe("SNS: Runtime config", function() {
    ...
    it("Validation failure silences the lambda", async() => {
        ...
        await expect( handler2( testSNSEvent, LambdaContext, () => {} ) ).rejects.toBeDefined()
    })
  ...
  it("Validation failure records exception", async () => {
    ...
    const handler2 = createSNSHandler(
      async (request) => {},
      {
        sources: {
          sns: {

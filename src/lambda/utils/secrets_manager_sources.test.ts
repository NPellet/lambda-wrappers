...
describe("Testing alternate sources in secrets manager", function() {
    ...
    const mgr = new LambdaFactoryManager().setAWSSecrets( awsSecrets ).addSecretSource<{}>()("GCP", otherSecrets, mockFetcher,  ( needsAws ) => {
    ...
    test("Secret from outside source is properly requested", async () => {
        ...
        const api = mgr.apiGatewayWrapperFactory("handler").needsSecret("GCP", "key", "DEF", "G", {}, true )

        const out = api.createHandler( class Ctrl {
            static async init() {
                return new Ctrl();
            }
    ...
    test("Pre-secrets are fetched when using an external secret source", async () => {
        ...
        const api = mgr.apiGatewayWrapperFactory("handler").needsSecret("GCP", "key", "DEF", "G", {}, true )
        const { configuration, handler } = api.createHandler( class Ctrl {
            static async init() {
                return new Ctrl();
            }
            async handler( data, secrets ) {
    ...
    test("Pre-secrets are NOT fetched when using an external secret source but dependent source is not required", async () => {
        ...
        const api = mgr.apiGatewayWrapperFactory("handler").needsSecret("aws", "key","ABC", "D", undefined, true )
        const { configuration, handler } = api.createHandler( class Ctrl {
            static async init() {
                return new Ctrl();
            }
